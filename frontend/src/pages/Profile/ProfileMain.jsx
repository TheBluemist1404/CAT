...
function ProfileMain({
  ...
  useEffect(() => {
    async function fetchSchools() {
      const data = await fetch(
        token,
        axios.get(
          `${import.meta.env.VITE_APP_API_URL}/api/v1/profile/detail/${
            user._id
          }`,
          {
            headers: {
  ...
  const handleSaveDescription = async () => {
    ...
    setDescription(data.description || unsavedDescription);
  ...
  return (
                ...
                <div style={{ display: "flex", alignItems: "center" }}>
                  ...
                  {information?.schools?.length > 0 && (
                    <div style={{ marginLeft: "10px" }}>
                      <div>
                        {information?.schools?.length === 1 && user.schools[0]}
                      </div>

                      <div>
                        {information?.schools[information?.schools?.length - 2]}
                      </div>
                    </div>
                  )}
                </div>
                    ...
                    <div style={{ marginLeft: "10px" }}>
                      <div>
                        {information?.companies?.length === 1 &&
                          information?.companies[0]}
                      </div>
                      <div>
                        {
                          information?.companies[
                            information?.companies?.length - 2
                          ]
                        }
                      </div>
                    </div>
              ...
              {showModal && (
                ...
                >
                  <div
                    ...
                    className="detail"
                  >
                    <h1 style={{ color: "#FFFFFF", marginTop: "10px" }}>
                      Editing Details
                    </h1>
    ...
    `}
                    ...
                    {[...companies, ...unsavedCompanies].map(
                      (company, index) => (
                        <div
                          key={index}
            ...
            <div className="im">
              ...
              <div className="flex-container">
                {Array.from({ length: 4 }).map((_, index) => {
                  const image = posts.flatMap((post) => post.images || [])[
                    index
                  ];
                  return (
                    <div
